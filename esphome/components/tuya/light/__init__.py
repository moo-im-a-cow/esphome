...
from esphome.const import CONF_OUTPUT_ID, CONF_MIN_VALUE, CONF_MAX_VALUE, CONF_GAMMA_CORRECT, \
    CONF_DEFAULT_TRANSITION_LENGTH, CONF_SWITCH_DATAPOINT,CONF_COLD_WHITE_COLOR_TEMPERATURE, CONF_WARM_WHITE_COLOR_TEMPERATURE
...
CONFIG_SCHEMA = cv.All(light.BRIGHTNESS_ONLY_LIGHT_SCHEMA.extend({
    ...
    cv.Optional(CONF_CCT_DATAPOINT): cv.uint8_t,
...
def to_code(config):
    ...
    if CONF_CCT_DATAPOINT in config:
